...
const AllDoctorsSelect2 = ({ navigation }) => {
    const [loader, setLoader] = useState(false);
    const [mainProviders, setMainProviders] = useState([]);
    ...
    useEffect(() => {
        ...
        async function getProviders() {
            try {
                ...
                setMainProviders(result?.data?.rows);
            } catch (error) {
                console.log(error);
            }
        }
        ...
        async function getServices() {
            try {
                if (search.length > 0) {
                    ...
                    setLoader(false);
                    setProviders(result?.data?.rows);
                } else setProviders(mainProviders);
            } catch (error) {
                console.log(error);
            }
        }
    ...
    return (
        <ScrollView w="100%">
            ...
            {loader == false && (
                <Center flex={1} w="100%" h="100%" justifyContent="center" pb={10}>
                    {providers?.length > 0 ? (
                        providers?.map(
                            (item, i) =>
                                item && (
